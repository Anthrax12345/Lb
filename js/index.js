...
// in libary object.
app.service("libaryService", ['$http', function ($http) {
			...
			this.addBook = function (newBook) {
				if (this.checkISBN === true) {
					...
					// DO AJAX CALL HERE.
					return this.persistDatatoFile();
				}

			};

			// AJAX CALL TO SAVE DATA AT SERVER SIDE.
			this.persistDatatoFile = function () {
				$http.post('/addsomedatatojson', this.libary).then(
					function (response) {
					return true;
				},
					function (response) {
...
app.controller("homepagecontroller", ["$location", "$scope", "libaryService", function ($location, $scope, libaryService) {
			...
			for (var i = 0; i < libaryService.books.length; i++) {
			...
			$scope.thClicked = function (i, k) {
				...
				for (var l = 0; l < $scope.thIcon.length; l++) {
					if (l != i) {
						$scope.thIcon[l] = "";
					}
...
app.controller("authordetailscontroller", ["$scope", "$location", "$mdDialog", "libaryService", function ($scope, $location, $mdDialog, libaryService) {
			...
			$scope.saveDetails = function () {
				...
				libaryService.persistDatatoFile();
			};
